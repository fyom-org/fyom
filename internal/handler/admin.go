...
func (h *AdminHandler) ListMedia(w http.ResponseWriter, r *http.Request) {
	...
	offset := (page - 1) * limit
	dataQuery := "SELECT " + repository.MediaColumns + " FROM media_items" + where + " ORDER BY " + orderBy + " LIMIT ? OFFSET ?"
...
func (h *AdminHandler) ListMissing(w http.ResponseWriter, r *http.Request) {
	...
	libraryID := r.URL.Query().Get("library_id")
	query := "SELECT " + repository.MediaColumns + " FROM media_items WHERE status = 'missing'"
	var args []interface{}
	if libraryID != "" {
